    ...
    @foreach($chatsUsers as $chatUser)
        ...
        <li class="flex py-4 pl-8 {{ ($chat == $chatUser->id ) ? 'bg-gray-100' : 'hover:bg-gray-100'}}" wire:click="changeChatUser({{$chatUser->id}}, {{($chatUser->user_id_one == \Auth::user()->id) ? $chatUser->userTwo->id : $chatUser->userOne->id}}, {{$messageNotRead}})" style="cursor:pointer;">
            @if($chatUser->userOne->profile_photo_path)
                <img class="h-12 w-12 rounded-full object-cover" src="{{($chatUser->user_id_one == \Auth::user()->id) ? $chatUser->userTwo->profile_photo_path : $chatUser->userOne->profile_photo_path}}" alt="{{$chatUser->userOne->name}}" width="150">
            @else
                <img class="h-12 w-12 rounded-full object-cover" src="{{($chatUser->user_id_one == \Auth::user()->id) ? $chatUser->userTwo->profile_photo_url : $chatUser->userOne->profile_photo_url}}" alt="{{$chatUser->userOne->name}}" width="150">
            @endif   
            <div class="w-3/4 pl-4">
                <p class="font-normal text-sm" style="font-weight: 450;">
                ...
                <div class="flex w-full">
                    <div class="font-normal text-sm text-gray-400 w-3/4">
                        {{$content}}
